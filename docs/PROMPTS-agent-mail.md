...
CONTEXTE PROJET :
- L'agent s'exécute sur serveur-local (10.0.0.XXX), un serveur Linux du réseau local.
...
4. Le dashboard n'a PAS d'authentification (LAN = confiance).
   Il bind 10.0.0.XXX uniquement, jamais 0.0.0.0.
...
CONTEXTE :
- Serveur : serveur-local (10.0.0.XXX)
...
- Cible : serveur-local avec KVM activé (/dev/kvm doit exister)
...
- Cible : serveur-local (10.0.0.XXX)
...
4. Écrire scripts/backup.sh :
   ...
   - Copie vers NAS-local (scp/rsync), rétention 30 jours

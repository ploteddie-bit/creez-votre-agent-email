...
## 1. Infrastructure (sur serveur-local)
...
|---------|------|----------|
| `gmail-credentials.json` | OAuth2 client secret Google Cloud | `/home/eddie/email-learner/configs/` (gitignored) |
| `token.json` | Refresh token OAuth2 | Généré au premier `--setup-oauth`, gitignored |
| `config.yaml` | Configuration applicative | `/home/eddie/email-learner/configs/` |

**Scope Gmail OAuth2 :** `https://www.googleapis.com/auth/gmail.modify`

Récupération des credentials : depuis `kimi-rag` ou `sg-rag`.
...
|--------|-------------|------|-----------|------|
| serveur-local | Gmail API | 443 | HTTPS | Sync emails |
| serveur-local | accounts.google.com | 443 | HTTPS | OAuth2 refresh |
| LAN (10.0.0.x) | serveur-local | 8080 | HTTPS | Dashboard |
| serveur-local (localhost) | serveur-local | 5432 | PostgreSQL | DB |
| serveur-local (localhost) | serveur-local | 11434 | HTTP | Ollama API |
